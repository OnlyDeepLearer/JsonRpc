...
public class CustomHttpStatusCodeProvider implements HttpStatusCodeProvider {
    ...
    public int getHttpStatusCode(int resultCode) {
        if (resultCode == 0) {
            return 200;
        } else if (resultCode == -32400 || resultCode == 400) {
            return 400;
        } else if (resultCode == -32404 || resultCode == 404) {
            return 404;
        } else if (resultCode == -32401 || resultCode == 401) {
            return 401;
        } else if (resultCode == 504) {
            return 504;
        } else if (resultCode == 405) {
            return 405;
        } else if (resultCode == -32500 || resultCode == 500 || this.isErrorCode(resultCode)) {
            return 500;
        } else if (resultCode != ErrorResolver.JsonError.INVALID_REQUEST.code &&
                resultCode != ErrorResolver.JsonError.PARSE_ERROR.code) {
